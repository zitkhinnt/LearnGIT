...
@section('title', __("horserace::be_sidebar.mail_replace_edit"))
@section('content')
  
  <!-- breadcumb -->
  @section('page_title', __("horserace::be_sidebar.mail_replace_edit"))
  @section('breadcrumb_item')
    <li class="breadcrumb-item">
      <a href="{{ route('admin.mail_replace') }}"> {{ __("horserace::be_sidebar.mail_replace") }} </a>
    </li>
    <li class="breadcrumb-item">{{ __("horserace::be_sidebar.mail_replace_edit") }}</li>
  @endsection
  <!-- START PAGE CONTENT-->
        ...
        <div class="ibox ibox-fullheight">
          <div class="ibox-head">
            <div class="ibox-title">
              {{ __("horserace::be_sidebar.mail_replace_edit") }}
            </div>
          </div>
          <form class="form-horizontal" action="{{ route('admin.mail_replace.store') }}"
                method="POST">
            @csrf
            <input type="hidden" name="id" value="{{ $data['mail_replace']->id }}">
            <div class="ibox-body">
              <div class="form-group mb-4 col-md-6">
                <label>{{ __("horserace::be_form.mail_replace_name") }}</label>
                <input class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}" type="text"
                       value="{{ $data['mail_replace']->name }}"
                       name="name">
                @if ($errors->has('name'))
              ...
              <div class="form-group mb-4 col-md-12">
                ...
                <textarea class="summernote" id="summernote" data-plugin="summernote" data-air-mode="true"
                          name="source"> {{ $data['mail_replace']->source }} </textarea>
              </div>
            </div>
