...
@section('title', __("horserace::be_sidebar.entrance_edit"))
@section('content')
...
@section('breadcrumb_item')
  ...
  <li class="breadcrumb-item">{{ __("horserace::be_sidebar.entrance_edit") }}</li>
@endsection
  ...
  <div class="ibox">
    ...
    <div class="ibox-body">
      ...
      <form action="{{ route('admin.entrance.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <input type="hidden" name="id" value="{{ $data["entrance"]->id }}">
        <div class="row">
          <!-- Name -->
          <div class="col-lg-4 form-group">
            <label>{{ __("horserace::be_form.entrance_name") }}</label>
            <input class="form-control {{ $errors->has('name') ? ' is-invalid' : '' }}"
                   type="text" name="name" value="{{ $data["entrance"]->name }}">
            @if ($errors->has('name'))
          ...
          <div class="col-sm-4 form-group">
            ...
            <input class="form-control {{ $errors->has('default_point') ? ' is-invalid' : '' }}"
                   type="number" name="default_point" value="{{ $data["entrance"]->default_point }}"
                   min="0">
            ...
            <select class="selectpicker show-tick form-control" name="default_user_stage">
              ...
              @foreach($data["user_stage"] as $item)
                <option value="{{ $item->id }}"
                  {{ $data["entrance"]->default_user_stage == $item->id ? "selected" : "" }}>
                  {{ $item->name }}
                </option>
              @endforeach
            </select>
            ...
            <div class="form-group">
              <label class="checkbox">
                <input type="checkbox" name="default_flg" value="{{ ENTRANCE_DEFAULT_ENABLE }}"
                  {{ $data["entrance"]->default_flg == ENTRANCE_DEFAULT_ENABLE ? "checked" : "" }}>
