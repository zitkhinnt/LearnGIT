...
@section('title', __("horserace::be_sidebar.payment_gift"))
@section('content')

<!-- breadcumb -->
@section('page_title', __("horserace::be_sidebar.payment_gift"))
@section('breadcrumb_item')
  <li class="breadcrumb-item">{{ __("horserace::be_sidebar.payment_gift") }}</li>
@endsection
      ...
      <div class="table-responsive row">
        <table class="table table-bordered table-hover" id="dataAjax">
          ...
          <tr>
            ...
            <th>{{ __("horserace::be_form.gift_name") }}</th>
          </tr>
          </thead>
          <tbody>
          @foreach($data["trans_gift"] as $item)
            <tr>
              <td>{{ $item->id }}</td>
  ...
  <script>
    $(function () {
      var table = $('#dataAjax').DataTable({
        ...
        "sAjaxSource": "{{ route('admin.trans_gift.ajax') }}",
        "iDisplayLength": 20,
        "aaSorting": [0, "desc" ],
        "aoColumns": [
          { "sName": "id"},
